...
# corners, four edge midpoints. insertion_point_offset() scales u and v by
# half the relevant dimensions (Width/Height for beams, Length/Width for
# columns) to place the point on the cross-section. Cycling order: center
# first, then clockwise from bottom-left.
_POINT_CANDIDATES = [
    (0, 0),  # 0: Center
    (-1, -1),  # 1: Bottom-Left
    (-1, 0),  # 2: Left-Mid
...
def insertion_point_offset(
    ...
    *rotation* must be the same value ``getPoint`` and ``update`` use to set the
    object's actual placement — typically the direct result of calling
    ``placement_rotation``.
